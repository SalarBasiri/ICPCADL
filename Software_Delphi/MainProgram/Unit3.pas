...
gain : integer;
sampling_frequency,time,dt : extended;
begin
...
begin
assignfile(fir_ch1,'c:/out_ch1.sal');
assignfile(fir_ch2,'c:/out_ch2.sal');
assignfile(fir_ch3,'c:/out_ch3.sal');
assignfile(fir_ch4,'c:/out_ch4.sal');
assignfile(fir_ch5,'c:/out_ch5.sal');
assignfile(fir_ch6,'c:/out_ch6.sal');
assignfile(fir_ch7,'c:/out_ch7.sal');
assignfile(fir_ch8,'c:/out_ch8.sal');
rewrite(final_output);
reset(fir_ch1,'c:/out_ch1.sal');
reset(fir_ch2,'c:/out_ch2.sal');
reset(fir_ch3,'c:/out_ch3.sal');
reset(fir_ch4,'c:/out_ch4.sal');
reset(fir_ch5,'c:/out_ch5.sal');
reset(fir_ch6,'c:/out_ch6.sal');
reset(fir_ch7,'c:/out_ch7.sal');
reset(fir_ch8,'c:/out_ch8.sal');
...
writeln(final_output,floattostr(roundto(time,-4))+' '+floattostr(roundto(out_data,-3)));
time := time+dt;
until (eof(fir_ch1));
end;
...
gain := strtoint(main_form.ch1_gain_cb.Items[main_form.ch1_gain_cb.itemindex]);
out_data := scale * in_data / gain;
write(final_output,floattostr(roundto(time,-4))+' '+floattostr(roundto(out_data,-3))+' ');
time := time+dt;
